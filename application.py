...
from models import db, Landlord, Tenant, Members, Admin
...
def load_user(user_id):
  try:
    return Landlord.query.filter_by(phone=user_id).first() or Tenant.query.filter_by(phone=user_id).first() or Members.query.filter_by(phone=user_id).first() or Admin.query.filter_by(phone=user_id).first()
  except:
    abort(500)
    # flash("Failed to login user", category="danger")
